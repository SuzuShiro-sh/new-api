...
func TestTokenLogDetailEnabledRoundTripsThroughRedisCache(t *testing.T) {
	useUserCacheMiniRedis(t)
	token := Token{
		Id:               42,
		UserId:           7,
		Key:              "token-log-detail-cache-key",
		Name:             "detail-cache",
	...
	require.NoError(t, err)
	assert.True(t, cached.LogDetailEnabled)
}
